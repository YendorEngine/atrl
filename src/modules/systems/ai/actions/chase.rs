...
pub fn chase_action(
    ...
    for (Actor(actor), mut action_state, mut chase) in action_q.iter_mut() {
        ...
        let position = if entity_in_fov(
            ...
            // Our pathfinder will only generate a valid path to the last seen location, this
            // includes partial path. We can expect the first element in the path
            // to be a valid location that is closest to the last_seen_pt.
            if !chase.generated_path {
                let path = PathFinder::Astar.compute(
                    ai_position,
                    last_seen,
                    &mut map_manager,
                    PathPassThroughData {
